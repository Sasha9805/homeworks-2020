...
$(document).ready(function() {
  var modalTitle = $('.modal-dialog').children().eq(1);
  var modalTitleText = $('.modal-dialog').children().eq(1).html();
  $('#btn').click(function() {
  ...
  var offerForm = $('#offer-form').validate({
    rules: {
      "username-offer": {
        required: true,
        minlength: 2,
    ...
    if (offerForm.form()) {
      $.ajax({
        ...
        success: function (data) {
          ...
          modalTitle.html(data + ', спасибо за заявку, скоро мы вам перезвоним.');
          $('#modal').addClass('modal_visible');
